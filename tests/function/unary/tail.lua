...
return section("]", function()
	it("gets everything but first element of lists", function()
		test.assert("[]", "],1")
		test.assert("[2]", "]+,1 ,2")
		test.assert("[2,3]", "]+@123")
		test.assert("[1,1,1]", "]*,1 4")
	end)

	it("gets everything but first character of strings", function()
		test.assert("", "]'a'")
		test.assert("bc", "]'abc'")
		test.assert("bcdef", "]+'abc' 'def'")
		test.assert("bc", "]+'' 'abc'")
		test.assert("bcabcabc", "]*'abc' 3")
	end)
	...
	it("does not take tail on empty lists or strings (empty list)", function()
		test.refute("] @")
		test.refute('] ""')
	end)
