...
std::string line_gutter(uint32_t line) { return std::to_string(line) + " | "; }
...
void format(std::string_view src, diagnostic dg) {
  ...
  length = std::min(length, (uint32_t)code.size() - start);

  std::string gutter = line_gutter(line);
  std::string empty = empty_gutter(gutter);

  std::string arrow = caret(code, column, length);
  croak(dg.level, dg.id, src.substr(start, length));

  std::cerr << empty << std::endl;
  std::cerr << gutter << code << std::endl;
  std::cerr << empty << arrow << " - here" << std::endl;
}

bool croak(level lvl, message id) {
  std::string_view message = messages[static_cast<uint8_t>(id)];
  std::string_view level = levels[static_cast<uint8_t>(lvl)];
...
void croak(std::string_view src, diagnostic dg) { format(src, dg); }
