...
void ir_successor_emit_phi(ir_function_t* function, ir_block_t* block, ir_block_t* jump, ir_instruction_t* phi, ir_id_t result) {
    if (block->successor_count == 0) {
        ir_instruction_t* instr = ir_emit(IR_JUMP, function, block);
        instr->jump.block = jump;
        
        if (phi->phi.phi_capacity <= phi->phi.phi_count) {
            phi->phi.phi_capacity *= 2;
            phi->phi.phi_values = arena_realloc(function->arena, phi->phi.phi_values, sizeof(ir_id_t) * phi->phi.phi_capacity);
        }

        phi->phi.phi_values[phi->phi.phi_count++] = result;
        phi->phi.phi_blocks[phi->phi.phi_count - 1] = block;
        ...
        for (int i = 0; i < block->successor_count; ++i) {
            ...
            if (succ) {
                ir_successor_emit_phi(function, succ, jump, phi, result);
            }
        }
    }
}
...
void ir_successor_return(ir_function_t* function, ir_block_t* block, ir_id_t result) {
    if (block->successor_count == 0) {
        ir_instruction_t* instr = ir_emit(IR_RETURN, function, block);
        instr->generic.operand_count = 1;
        instr->generic.operands = malloc(sizeof(ir_id_t));
        instr->generic.operands[0] = result;
        block->successor_count = -1;
    } else {
        for (int i = 0; i < block->successor_count; ++i) {
            ir_block_t* succ = NULL;
            for (int j = 0; j < function->block_count; ++j) {
                if (function->blocks[j]->id == block->successors[i]) {
                    succ = function->blocks[j];
                    break;
                }
            }
            ...
            if (succ) {
                ir_successor_return(function, succ, result);
            }
        }
    }
}
        ...
        switch (node->kind) {
            ...
            case AST_BLOCK:
                if (item->state == 0) {
                    ...
                    instr->block.function = generated_block;
                    ...
                    ir_successor_return(function, node->instruction->block.function, node->arg1->result);
                }
                break;
            case AST_CALL:
                if (item->state == 0) {
            ...
            case AST_AND:
                if (item->state == 0) {
                    ...
                    ir_block_t* truthy = ir_create_block(function);
                    ...
                    instr->branch.falsey = merge_block;

                    ir_successor_add(block, truthy->id);
                    ir_successor_add(block, merge_block->id);

                    ir_worklist_add(worklist, node, block, 2);
                    ir_worklist_add(worklist, node->arg2, truthy, 0);

                    node->instruction = instr;
                } else {
                    ir_block_t* merge_block = node->instruction->branch.falsey;

                    ir_instruction_t* phi = ir_emit(IR_PHI, function, merge_block);
                    phi->phi.phi_count = 1;
                    phi->phi.phi_capacity = 2;
                    phi->phi.phi_values = arena_alloc(function->arena, sizeof(ir_id_t) * 2);
                    phi->phi.phi_blocks = arena_alloc(function->arena, sizeof(ir_block_t) * 2);
                    phi->phi.phi_values[0] = node->arg1->result;
                    phi->phi.phi_blocks[0] = block;
                    node->result = phi->result;

                    ir_successor_emit_phi(function, node->instruction->branch.truthy, merge_block, phi, node->arg2->result);
                    ...
                    for (size_t i = 0; i < worklist->size; ++i) {
                        ...
                        } else if (worklist->items[i]->node->block == block) {
                            worklist->items[i]->node->block = merge_block;
                        }
                    }
                }
                break;
            case AST_OR:
                if (item->state == 0) {
                    ...
                    instr = ir_emit(IR_BRANCH, function, block);
                    instr->branch.condition = node->arg1->result;
                    instr->branch.truthy = merge_block;
                    instr->branch.falsey = truthy;
                    ...
                    ir_block_t* merge_block = node->instruction->branch.truthy;
                    ...
                    ir_successor_emit_phi(function, node->instruction->branch.falsey, merge_block, phi, node->arg2->result);
            ...
            case AST_IF:
                if (item->state == 0) {
                    ...
                    ir_successor_emit_phi(function, node->instruction->branch.truthy, merge_block, phi, node->arg2->result);
                    ir_successor_emit_phi(function, node->instruction->branch.falsey, merge_block, phi, node->arg3->result);
                    ir_successor_add(node->instruction->branch.truthy, merge_block->id);
                    ir_successor_add(node->instruction->branch.falsey, merge_block->id);
            ...
            case AST_WHILE:
                if (item->state == 0) {
                    ...
                    ir_instruction_t* null = ir_emit(IR_CONST_NULL, function, block);
                    null->constant.value = v_create(TYPE_NULL, NULL);
                    node->result = null->result;

                    instr = ir_emit(IR_JUMP, function, block);
                    instr->jump.block = condition_block;
                    node->block = condition_block;
                    node->top = condition_block;
                    ...
                    ir_successor_emit(function, node->instruction->branch.truthy, node->top);

                    for (size_t i = 0; i < worklist->size; ++i) {
                        if (worklist->items[i]->block == block) {
                            worklist->items[i]->block = node->instruction->branch.falsey;
                        } else if (worklist->items[i]->node->block == block) {
                            worklist->items[i]->node->block = node->instruction->branch.falsey;
                        }
                    }
                }
                break;
